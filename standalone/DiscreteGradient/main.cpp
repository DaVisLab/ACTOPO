...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    parser.setArgument("m", &mode, "Work mode", true);
    parser.setArgument("c", &capacity, "Cache size", true);
    parser.setArgument("p", &prodNum, "Producer number", true);
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("DiscreteGradient");

  // ---------------------------------------------------------------------------
  // Initialize ttkDiscreteGradient module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto discreteGradient = vtkSmartPointer<ttkDiscreteGradient>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  discreteGradient->SetBufferSize(capacity);
  discreteGradient->SetWorkMode(mode);
  discreteGradient->SetComputeGradientGlyphs(false);
  discreteGradient->SetProducerNumber(prodNum);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkDiscreteGradient filter
      discreteGradient->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    discreteGradient->SetInputArrayToProcess(i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkDiscreteGradient filter
  // ---------------------------------------------------------------------------
  discreteGradient->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < discreteGradient->GetNumberOfOutputPorts(); i++) {
      auto output = discreteGradient->GetOutputDataObject(i);
      auto writer
