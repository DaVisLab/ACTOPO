...
vtkStandardNewMacro(ttkFormanGradient);
 ...
 */
ttkFormanGradient::ttkFormanGradient() {
 ...
 */
int ttkFormanGradient::FillInputPortInformation(int port,
                                                vtkInformation *info) {
  if(port == 0) {
 ...
 */
int ttkFormanGradient::FillOutputPortInformation(int port,
                                                 vtkInformation *info) {
  if(port == 0) {
 ...
 */
int ttkFormanGradient::RequestData(vtkInformation *request,
  ...
  //       call SetInputArrayToProcess (see FormanGradient.xml file).
  //
  ...
  ttkVtkTemplateMacro(inputArray->GetDataType(), triangulation->getType(),
                      (status = this->computeGradient<VTK_TT, TTK_TT>(
                         (VTK_TT *)ttkUtils::GetVoidPointer(outputArray),
                         (VTK_TT *)ttkUtils::GetVoidPointer(inputArray),
                         (TTK_TT *)triangulation->getData())));
  this->printMsg("Time usage for computing gradient: "
                 + std::to_string(t.getElapsedTime()) + " s.");
  ...
  this->printMsg("Memory usage: " + std::to_string(m.getValue_in_MB(false)) + " MB.");
