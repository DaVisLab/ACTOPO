...
RegistryTriangulation
  ...
  if(hasIndexArray) {
    this->printMsg("Initializing AcTopo", 0, 0,
                   ttk::debug::LineMode::REPLACE, ttk::debug::Priority::DETAIL);
  } else {
  ...
  if(hasIndexArray) {
    this->printMsg("Initializing AcTopo", 1, timer.getElapsedTime(),
...
ttk::Triangulation *
  ttkTriangulationFactory::GetTriangulation(
    int debugLevel, float ratio, int mode, int numProducer, vtkDataSet *object) {
  auto instance = &ttkTriangulationFactory::Instance;
  ...
  if(triangulation) {
    ...
    triangulation->setBufferSize(ratio);
    triangulation->setProducerNumber(numProducer);
    triangulation->setWorkMode(mode);
  }
