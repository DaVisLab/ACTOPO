...
namespace ttk {
  ...
  public:
    ...
    enum class Type { EXPLICIT, IMPLICIT, PERIODIC, ACTOPO };
    ...
    inline Triangulation::Type getType() const {
      ...
      else if(abstractTriangulation_ == &acTopo_)
        return Triangulation::Type::ACTOPO;
      else
    ...
    inline int setDebugLevel(const int &debugLevel) {
      ...
      acTopo_.setDebugLevel(debugLevel);
      debugLevel_ = debugLevel;
      return 0;
    }

    // Set the buffer size
    inline int setBufferSize(const float &ratio) {
      if(abstractTriangulation_ == &acTopo_) {
        acTopo_.setBufferSize(ratio);
      }
      return 0;
    }

    // Set the working mode for AcTopo 
    inline int setWorkMode(const int type) {
      if(abstractTriangulation_ == &acTopo_)
        acTopo_.setWorkMode(type);
      return 0;
    }

    // Set the number of producers for AcTopo 
    inline int setProducerNumber(const int num) {
      if(abstractTriangulation_ == &acTopo_)
        acTopo_.setProducerNumber(num);
      return 0;
    }
    ...
    inline int setStellarInputCells(const SimplexId &cellNumber,
                                    const LongSimplexId *connectivity,
                                    const LongSimplexId *offset) {
      abstractTriangulation_ = &acTopo_;
      gridDimensions_[0] = gridDimensions_[1] = gridDimensions_[2] = -1;
      return acTopo_.setInputCells(cellNumber, connectivity, offset);
    }
#else
    ...
    inline int setStellarInputCells(const SimplexId &cellNumber,
      ...
      abstractTriangulation_ = &acTopo;
      gridDimensions_[0] = gridDimensions_[1] = gridDimensions_[2] = -1;

      return acTopo.setInputCells(cellNumber, cellArray);
    }
#endif
    ...
    inline int setThreadNumber(const ThreadId threadNumber) {
      ...
      acTopo_.setThreadNumber(threadNumber);
